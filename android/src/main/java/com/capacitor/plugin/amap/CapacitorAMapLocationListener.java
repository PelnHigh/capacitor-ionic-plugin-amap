...
public class CapacitorAMapLocationListener implements AMapLocationListener, WeatherSearch.OnWeatherSearchListener {
    ...
    private List<CapacitorAmapLocation> locations = new ArrayList<>();
    ...
    public void onLocationChanged(AMapLocation aMapLocation) {
        if (aMapLocation != null && 0 == aMapLocation.getErrorCode()) {
            ...
            jsObject.put("locationType", aMapLocation.getLocationType());
            ...
            CapacitorAmapLocation item = new CapacitorAmapLocation();
            item.setAccuracy(aMapLocation.getAccuracy());
            item.setJsObject(jsObject);
            locations.add(item);
        ...
        this.capacitorAMap.isInLocation = false;
        CapacitorAmapLocation min = locations.stream().min(Comparator.comparing(CapacitorAmapLocation::getAccuracy)).get();

        resolve(min.getJsObject());
    }
